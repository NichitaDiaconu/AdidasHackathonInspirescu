...
function receivedMessage(event) {
  ...
  if (messageText) {
    var action = messageText.replace(/[^\w\s]/gi, '').trim().toLowerCase();
    ...
    SENDER_ID = senderID;
    var imageLink = messageAttachments[0].payload.url;
    var filePath = SERVER_URL + 'user_images/test.png';

    downloadImage(imageLink, filePath, processImage);
  }
} catch (err) {
  console.log(err.message);
}
...
function sendImage(callback, filePath) {
  console.log("path3: "+filePath);
  var command = "curl  \
  -F 'message={\"attachment\":{\"type\":\"image\", \"payload\":{\"is_reusable\":true}}}' \
  -F 'filedata=@"+filePath+";type=image/png' \
  ...
  function (error, stdout, stderr) {
      ...
      callback(SENDER_ID, response.attachment_id)
  });
}

function transformImage(callback, sendImageMessage, filePath) {
  ...
  if (random == 0) {
    random = 1;
    var style = 'wave';
    var command = 'cd ./../../fast-style-transfer; \
      . /home/nichita/environments/TF_env/bin/activate ; \
      python2 evaluate.py --checkpoint ./models/'+style+'.ckpt \
    --in-path ./../AdidasInspirescu/node/user_images/ \
    --out-path ./../AdidasInspirescu/node/output_images/; \
    python2 ./../add_filter.py ./../AdidasInspirescu/node/output_images/test.png ./../filter.png ./../AdidasInspirescu/node/output_images/test.png;';
  }
  else if (random == 1){
    random = 2;
    var style = 'scream';
    ...
    var style = 'scream';
    var command = 'cd ./../../fast-style-transfer; \
    python2 ./../add_filter.py ./../AdidasInspirescu/node/user_images/test.png ./../h2c.png ./../AdidasInspirescu/node/user_images/test.png; \
    . /home/nichita/environments/TF_env/bin/activate ; \
    python2 evaluate.py --checkpoint ./models/'+style+'.ckpt \
    --in-path ./../AdidasInspirescu/node/user_images/ \
    --out-path ./../AdidasInspirescu/node/output_images/; \
  ...
  function (error, stdout, stderr) {
      ...
      console.log("path2: "+filePath);
      console.log(stdout);
      callback(sendImageMessage, filePath);
  });
}

function downloadImage(imageLink, filePath, callback) {
  var fs = require('fs'),
  request = require('request');


  var download = function(uri, filePath){
  ...
  download(imageLink, filePath, callback);
  console.log("HERE1");
}

function processImage() {

    // MODIFY IMAGE WITH NICHITA STUFF
    var filePath = SERVER_URL + 'output_images/test.png';
    transformImage(sendImage, sendImageMessage, filePath);

    //sendImage(sendImageMessage, filePath);
}
/*
 ...
 *
 */
function sendImageMessage(recipientId, attachmentId) {
